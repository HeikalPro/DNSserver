...
print(f"Encoded TXT value: {base64.urlsafe_b64encode(encrypted_value).decode('utf-8')}")
print(f"Encrypted TXT value: {encrypted_value}")
...
dns_records = {
    ...
    'nyu.edu.': {
        ...
        dns.rdatatype.SOA: (
            'ns1.nyu.edu.', 'admin.nyu.edu.', 2023081401, 3600, 1800, 604800, 86400
        )
    },
    'legitsite.com.': {
...
# DNS Client Function
def query_dns_server(server_ip, query_name, query_type):
    query = dns.message.make_query(query_name, query_type)
    response = dns.query.udp(query, server_ip)

    print(f"Querying {query_name} ({query_type}) from {server_ip}")
...
if __name__ == '__main__':
    # Start DNS Server
    threading.Thread(target=run_dns_server_user).start()
    ...
    query_dns_server(server_ip, query_name, query_type)
